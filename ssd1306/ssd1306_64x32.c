...
#define DW_HI(PORT) digitalWrite(PORT, HIGH);
#define DW_LO(PORT) digitalWrite(PORT, LOW);
...
void ssd1306_send_byte(uint8_t byte)
{
  ...
  for (i = 0; i < 8; i++)
  {
  	digitalWrite(SSD1306_SDA, (byte << i) & 0x80);
...
void ssd1306_init ()
{
  pinMode(SSD1306_SDA, OUTPUT);
  pinMode(SSD1306_SCL, OUTPUT);
...
void ssd1306_setpos (uint8_t col, uint8_t page)
{
  ...
  ssd1306_send_byte(0); //Just this page (I haven't figured out how to config for auto-wrap)
  ssd1306_send_command_stop();
}
